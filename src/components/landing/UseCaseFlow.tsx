...
const scenarios_ko: Scenario[] = [
  { icon: Code2, headline: '백엔드 개발자인데\n대표님이 기획안을 써오라고 했다', before: '뭘 써야 할지 모르겠다. 기획은 내 전문이 아닌데.', after: '기획안 구조가 잡혔다. 대표님이 뭘 보고 싶은지도 파악됐다.', color: '#2d4a7c', demoId: 'planning' },
  { icon: ClipboardList, headline: 'PM인데 전략 제안서를\n2시간 안에 내야 한다', before: '시간이 없다. 어디서부터 시작해야 하지.', after: '핵심만 뽑아서 설득력 있게 정리됐다. 약점도 미리 파악했다.', color: '#6b4c9a', demoId: 'proposal' },
  { icon: Palette, headline: '디자이너인데\n비즈니스 케이스를 만들라고 했다', before: '비즈니스 언어를 모르겠다. ROI가 뭐지.', after: '숫자와 논리로 번역됐다. 경영진이 이해하는 언어로.', color: '#2d6b2d', demoId: 'strategy' },
];

const scenarios_en: Scenario[] = [
  { icon: Code2, headline: "Backend developer asked\nto write a project proposal", before: "No idea where to start. Planning isn't my thing.", after: 'Proposal structure is ready. Now I know what the CEO actually wants to see.', color: '#2d4a7c', demoId: 'planning' },
  { icon: ClipboardList, headline: "PM with a strategy deck\ndue in 2 hours", before: "No time. Where do I even begin?", after: "Key points extracted and structured persuasively. Weak spots caught early.", color: '#6b4c9a', demoId: 'proposal' },
  { icon: Palette, headline: "Designer told to build\na business case", before: "I don't speak business. What even is ROI?", after: "Translated into numbers and logic. In a language execs understand.", color: '#2d6b2d', demoId: 'strategy' },
];
      ...
      <div className="relative max-w-5xl mx-auto px-5 md:px-6 py-12 md:py-16">
        <div className="text-center mb-8 md:mb-10">
          <h2 className="text-display-lg text-[var(--text-primary)]">
            {L('막막함에서 ', 'From stuck ')}
            <span className="text-gold-gradient">{L('제출 가능한 문서까지', 'to ready-to-submit')}</span>
          </h2>
          <p className="mt-2 text-[13px] md:text-[15px] text-[var(--text-secondary)]">
            {L('직군이 달라도, 결과물의 수준은 같습니다', 'Same output quality, regardless of your background')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
          {scenarios.map((s, i) => (
            <RevealCard key={i} delay={i * 120}>
              <Link
                href={`/workspace?demo=${s.demoId}`}
                onClick={() => track('landing_usecase_click', { scenario: s.demoId })}
                className="block h-full rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg)] p-5 md:p-6 shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] hover:-translate-y-1.5 transition-all duration-300 group"
              ...
              >
                ...
                {/* Click hint */}
                <div className="flex items-center gap-1 mt-4 text-[12px] text-[var(--accent)] font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  {L('이 상황으로 체험해보기', 'Try this scenario')}
