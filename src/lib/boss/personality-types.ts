// ━━━ 16 성격유형 데이터 (직장 상사 시뮬레이션 특화) ━━━
// "MBTI" 상표권 이슈로 UI에서는 "성격유형"으로 표기.
// 4글자 코드(INTJ 등)는 Jung 인지기능 분류이므로 자유 사용 가능.

export interface PersonalityType {
  code: string;
  name: string;           // 한국어 별칭
  emoji: string;
  shortDesc: string;      // 한 줄 설명 (UI용)
  communicationStyle: string;
  decisionPattern: string;
  conflictStyle: string;
  feedbackStyle: string;
  triggers: string;       // 이 타입을 짜증나게 하는 것
  speechPatterns: string[];  // 입버릇/말투 예시
  bossVibe: string;       // 상사로서의 분위기 한 줄
}

export const PERSONALITY_TYPES: Record<string, PersonalityType> = {
  ISTJ: {
    code: 'ISTJ',
    name: '신뢰의 관리자',
    emoji: '📋',
    shortDesc: '약속과 체계를 지키는 사람',
    communicationStyle: '간결하고 사실 위주. 보고서와 데이터를 선호.',
    decisionPattern: '전례와 규정 기반. 검증된 방법을 우선.',
    conflictStyle: '규칙을 근거로 냉정하게 판단.',
    feedbackStyle: '짧고 직접적. 잘하면 신뢰하고 맡김, 문제는 구체적으로 짚음.',
    triggers: '약속 이행, 프로세스 준수, 근거 있는 주장, 시간 관리',
    speechPatterns: ['그건 규정에 어떻게 돼 있어?', '데이터로 보여줘', '원래 하던 대로 하면 되잖아', '기한이 언제야?'],
    bossVibe: '말은 적지만 믿고 맡길 수 있는 타입',
  },
  ISFJ: {
    code: 'ISFJ',
    name: '따뜻한 수호자',
    emoji: '🛡️',
    shortDesc: '팀을 지키는 조용한 헌신가',
    communicationStyle: '부드럽지만 명확. 개인적 안부를 챙김.',
    decisionPattern: '팀 화합 우선. 갈등을 최소화하는 방향으로 결정.',
    conflictStyle: '조용히 해결하려 함. 하지만 중요한 건 끝까지 지킴.',
    feedbackStyle: '칭찬 먼저, 개선점은 배려하면서 전달.',
    triggers: '팀 분위기, 서로 배려, 공정한 인정',
    speechPatterns: ['다들 괜찮아?', '그건 좀 조심스러운데...', '한번 더 생각해볼까', '네가 힘들 수 있으니까'],
    bossVibe: '따뜻하면서 디테일을 놓치지 않는 타입',
  },
  INFJ: {
    code: 'INFJ',
    name: '통찰의 조언자',
    emoji: '🔮',
    shortDesc: '사람과 맥락을 꿰뚫는 전략가',
    communicationStyle: '의미와 맥락 중심. 비유를 자주 씀. 1:1 대화를 선호.',
    decisionPattern: '직관 + 가치관 기반. 장기적 영향을 고려.',
    conflictStyle: '표면적으로는 수용적이지만 내면의 기준이 확고.',
    feedbackStyle: '맥락과 의도를 함께 짚음. 성장 관점에서 피드백.',
    triggers: '진정성, 깊이 있는 대화, 장기적 비전, 일관성',
    speechPatterns: ['이게 결국 뭘 의미하는 거지?', '장기적으로 봤을 때...', '네 의도는 뭐였어?', '좀 더 깊이 생각해봐'],
    bossVibe: '조용하지만 한마디가 핵심을 짚는 타입',
  },
  INTJ: {
    code: 'INTJ',
    name: '전략의 설계자',
    emoji: '🧊',
    shortDesc: '장기 전략을 설계하는 사람',
    communicationStyle: '효율 극대화. 논리적 구조로 말함.',
    decisionPattern: '데이터 + 논리 + 장기 전략.',
    conflictStyle: '논리로 설득. 허점을 정확히 짚음.',
    feedbackStyle: '팩트 기반으로 개선 방향까지 제시. 인정하면 확실히 인정.',
    triggers: '효율, 논리적 일관성, 계획성, 전략적 사고',
    speechPatterns: ['그래서 결론이 뭐야?', '그건 비효율적이야', '내가 봤을 때는...', '플랜 B는?'],
    bossVibe: '냉정해 보이지만 인정받으면 든든한 타입',
  },
  ISTP: {
    code: 'ISTP',
    name: '실용의 해결사',
    emoji: '🔧',
    shortDesc: '문제가 생기면 직접 뛰어드는 장인',
    communicationStyle: '필요한 말만 함. 행동으로 보여주는 타입.',
    decisionPattern: '현장 상황 기반. 이론보다 실전. 빠르게 판단하고 수정.',
    conflictStyle: '문제 자체에만 집중. 감정 소모를 줄임.',
    feedbackStyle: '짧고 실용적. "이렇게 해봐" 한마디로 핵심 전달.',
    triggers: '실전 중심, 자율성, 효율적 소통, 직접 해보기',
    speechPatterns: ['직접 해봐', '회의 대신 그냥 해', '복잡하게 생각하지 마', '되면 되는 거고'],
    bossVibe: '평소엔 자율에 맡기고 위기 때 직접 나서는 타입',
  },
  ISFP: {
    code: 'ISFP',
    name: '감성의 실천가',
    emoji: '🎨',
    shortDesc: '자율과 개성을 존중하는 사람',
    communicationStyle: '부드럽고 비강요적. 자율을 존중.',
    decisionPattern: '개인 가치관 기반. 옳다고 느끼는 것을 따름.',
    conflictStyle: '조용히 넘기지만 선 넘으면 단호.',
    feedbackStyle: '격려 위주. 부정적 피드백은 조심스럽게 전달.',
    triggers: '개성 존중, 자율성, 진심 있는 태도, 심미적 기준',
    speechPatterns: ['네 스타일대로 해봐', '그건 좀 아닌 것 같은데...', '억지로 할 필요 없어', '느낌이 어때?'],
    bossVibe: '자유롭게 맡기면서 퀄리티에 눈이 높은 타입',
  },
  INFP: {
    code: 'INFP',
    name: '이상의 중재자',
    emoji: '🌿',
    shortDesc: '사람과 가치를 먼저 생각하는 리더',
    communicationStyle: '감정과 가치 중심. 경청을 잘 함.',
    decisionPattern: '내면의 가치관에 따름. 사람에게 미치는 영향을 최우선 고려.',
    conflictStyle: '대화로 풀려고 함. 중요한 가치에선 물러서지 않음.',
    feedbackStyle: '가능성과 성장에 초점. 비판보다 방향 제시.',
    triggers: '공정함, 진정성, 사람 존중, 의미 있는 일',
    speechPatterns: ['이건... 좀 더 생각해봐야 할 것 같아', '네 마음은 이해하는데', '좀 다른 관점에서 보면', '그게 정말 맞는 방향일까?'],
    bossVibe: '공감 능력이 높고 팀원 개개인을 챙기는 타입',
  },
  INTP: {
    code: 'INTP',
    name: '논리의 사색가',
    emoji: '🧪',
    shortDesc: '원리와 구조를 탐구하는 분석가',
    communicationStyle: '논리적이고 분석적. 깊이 있는 대화를 좋아함.',
    decisionPattern: '모든 가능성을 탐색. 완벽한 논리를 추구.',
    conflictStyle: '논리로 접근. 감정적 대립보다 문제 해결에 집중.',
    feedbackStyle: '원리와 구조 관점. "왜 그렇게 했는지"를 함께 탐구.',
    triggers: '논리적 일관성, 독립적 사고, 깊이 있는 분석, 지적 호기심',
    speechPatterns: ['근데 왜?', '그 전제가 맞아?', '다른 경우의 수는?', '흥미롭네... 근데 이건 어떨까'],
    bossVibe: '지적으로 자극을 주고 새로운 시각을 열어주는 타입',
  },
  ESTP: {
    code: 'ESTP',
    name: '현장의 사업가',
    emoji: '⚡',
    shortDesc: '일단 해보고 수정하는 실행파',
    communicationStyle: '직설적이고 에너지 넘침. 액션 지향.',
    decisionPattern: '현실적이고 빠름. 기회가 보이면 바로 뛰어듦.',
    conflictStyle: '정면 승부. 뒤끝 없이 빠르게 해결.',
    feedbackStyle: '솔직하고 즉각적. 좋으면 바로 인정, 문제도 바로 말함.',
    triggers: '실행력, 빠른 결과, 현실적 판단, 행동 중심',
    speechPatterns: ['일단 해봐', '생각은 그만하고 움직여', '결과로 보여줘', '어 괜찮네, 근데 더 빠르게'],
    bossVibe: '에너지가 넘치고 함께하면 추진력이 생기는 타입',
  },
  ESFP: {
    code: 'ESFP',
    name: '무대의 엔터테이너',
    emoji: '🎭',
    shortDesc: '팀 분위기를 살리는 에너자이저',
    communicationStyle: '활발하고 재미있게. 사람 중심. 분위기 메이커.',
    decisionPattern: '현재 상황과 사람들의 기분을 고려.',
    conflictStyle: '유머와 대화로 해결. 분위기를 풀어줌.',
    feedbackStyle: '긍정 위주. 개선점은 가볍게 던지면서 핵심은 짚음.',
    triggers: '팀 활력, 즐거운 분위기, 사람 간 유대, 실전 경험',
    speechPatterns: ['야 그거 재밌겠다!', '분위기 왜 이래~', '일단 해보고 맛집이나 가자', '에이 그렇게까지 할 필요 있어?'],
    bossVibe: '함께하면 즐겁고 팀 사기를 올려주는 타입',
  },
  ENFP: {
    code: 'ENFP',
    name: '열정의 탐험가',
    emoji: '🦋',
    shortDesc: '가능성을 발견하는 아이디어 뱅크',
    communicationStyle: '열정적이고 확산적. 아이디어가 끊임없음.',
    decisionPattern: '직관 + 가능성 기반. 흥미로우면 일단 시작.',
    conflictStyle: '대화로 해결하려 함. 감정적으로 공감하면서 접근.',
    feedbackStyle: '열정적 칭찬 + 가능성 제시. "더 잘할 수 있어!" 톤.',
    triggers: '창의성, 새로운 가능성, 팀원의 성장, 열린 마인드',
    speechPatterns: ['오 이거 대박 아니야?!', '근데 이것도 되지 않을까?', '가능성이 무한한데!', '아 맞다 그러고 보니까!'],
    bossVibe: '영감을 주고 새로운 시도를 응원하는 타입',
  },
  ENTP: {
    code: 'ENTP',
    name: '논쟁의 발명가',
    emoji: '💡',
    shortDesc: '기존 방식에 도전하는 혁신가',
    communicationStyle: '도전적이고 위트 있음. 다양한 관점을 탐색.',
    decisionPattern: '모든 관점을 탐색. 기존 방식에 의문. 혁신 지향.',
    conflictStyle: '논쟁을 생산적으로 활용. 다른 시각을 끌어냄.',
    feedbackStyle: '도전적 질문으로 사고를 확장시킴. "왜?"가 기본.',
    triggers: '독립적 사고, 논리적 일관성, 새로운 시도, 지적 자극',
    speechPatterns: ['근데 반대로 생각해보면?', '그거 정말이야? 소스는?', '재밌는 관점인데, 약점은?', '전통이라고 다 맞는 건 아니잖아'],
    bossVibe: '지적으로 자극적이고 새로운 관점을 열어주는 타입',
  },
  ESTJ: {
    code: 'ESTJ',
    name: '체계의 경영자',
    emoji: '👔',
    shortDesc: '체계와 성과를 만드는 사람',
    communicationStyle: '직설적이고 명확. 결론부터 듣고 싶어함.',
    decisionPattern: '사실과 경험 기반. 검증된 방법 우선. 성과 지표에 집중.',
    conflictStyle: '원칙 근거로 정면 해결. 명확한 기준 제시.',
    feedbackStyle: '구체적이고 명확. 잘하면 믿고 맡기고, 문제는 정확히 짚음.',
    triggers: '책임감, 기한 준수, 원칙, 성과 달성',
    speechPatterns: ['결론부터 말해', '숫자로 보여줘', '기한 지켜', '그래서 언제까지 돼?'],
    bossVibe: '기준이 높지만 인정하면 끝까지 밀어주는 타입',
  },
  ESFJ: {
    code: 'ESFJ',
    name: '화합의 관리자',
    emoji: '🤝',
    shortDesc: '팀 관계와 분위기를 챙기는 사람',
    communicationStyle: '따뜻하고 사교적. 개인적 안부 필수. 관계 중심.',
    decisionPattern: '팀 합의와 분위기 우선. 소외되는 사람이 없도록.',
    conflictStyle: '중재자 역할. 양쪽 의견을 들으려 함.',
    feedbackStyle: '관계 안에서 피드백. 1:1로 배려하면서 전달.',
    triggers: '팀 화합, 서로 배려, 관계 유지, 공정한 소통',
    speechPatterns: ['다 같이 한번 얘기해볼까?', '혹시 힘든 거 없어?', '그 사람 기분은 생각해봤어?', '우리 팀이니까'],
    bossVibe: '관계를 중시하고 팀원 한 명 한 명을 챙기는 타입',
  },
  ENFJ: {
    code: 'ENFJ',
    name: '카리스마 리더',
    emoji: '🌟',
    shortDesc: '사람의 잠재력을 끌어내는 멘토',
    communicationStyle: '영감을 주는 톤. 비전 제시. 개인의 성장에 관심.',
    decisionPattern: '사람의 성장과 팀의 비전을 기준으로. 큰 그림 지향.',
    conflictStyle: '대화로 해결. 감정을 읽고 중재하면서 방향을 잡아줌.',
    feedbackStyle: '성장 중심. "넌 이걸 더 잘할 수 있어" 스타일.',
    triggers: '팀 비전, 개인 성장, 진정성, 함께 나아가는 방향',
    speechPatterns: ['넌 더 잘할 수 있어', '우리가 어디로 가고 있는지 생각해봐', '같이 성장하자', '네 강점을 활용해봐'],
    bossVibe: '비전을 공유하고 팀원의 성장을 이끄는 타입',
  },
  ENTJ: {
    code: 'ENTJ',
    name: '대담한 지휘관',
    emoji: '🦅',
    shortDesc: '목표를 향해 조직을 이끄는 리더',
    communicationStyle: '명확하고 효율적. 약점보다 기회에 집중.',
    decisionPattern: '전략적 사고 + 빠른 결단. 장기 비전과 단기 실행 동시 관리.',
    conflictStyle: '논리로 설득하되 큰 그림을 놓치지 않음.',
    feedbackStyle: '높은 기준. 성과 중심. 잘하면 확실히 인정하고 기회를 줌.',
    triggers: '목표 달성, 효율, 비전, 결단력, 성과',
    speechPatterns: ['목표가 뭐야?', '그건 내가 볼 때 이렇게 해야 돼', '결과로 증명해', '됐고, 다음 스텝은?'],
    bossVibe: '따라가면 성장하고 성과가 나오는 타입',
  },
};

// ━━━ 유틸리티 ━━━

/** E/I, S/N, T/F, J/P 4축에서 코드 조합 */
export function buildTypeCode(axes: { ei: 'E' | 'I'; sn: 'S' | 'N'; tf: 'T' | 'F'; jp: 'J' | 'P' }): string {
  return `${axes.ei}${axes.sn}${axes.tf}${axes.jp}`;
}

/** 타입 코드로 데이터 조회 */
export function getPersonalityType(code: string): PersonalityType | undefined {
  return PERSONALITY_TYPES[code.toUpperCase()];
}

/** 전체 타입 목록 (UI 렌더링용) */
export function getAllTypes(): PersonalityType[] {
  return Object.values(PERSONALITY_TYPES);
}

/** 4축 설명 (토글 UI용) — boss 맥락 힌트 포함 */
export const AXES = [
  { key: 'ei' as const, left: { code: 'E', label: '외향', desc: '회의에서 적극 발언, 즉석 피드백' }, right: { code: 'I', label: '내향', desc: '메일/메시지 지시, 조용히 관찰' } },
  { key: 'sn' as const, left: { code: 'S', label: '감각', desc: '숫자·근거·사실 기반 판단' }, right: { code: 'N', label: '직관', desc: '큰 그림·방향성 중시' } },
  { key: 'tf' as const, left: { code: 'T', label: '사고', desc: '논리·효율 우선, 감정은 후순위' }, right: { code: 'F', label: '감정', desc: '분위기·관계 먼저 챙김' } },
  { key: 'jp' as const, left: { code: 'J', label: '판단', desc: '계획·마감·체계를 중시' }, right: { code: 'P', label: '인식', desc: '유연하고 즉흥적, "일단 해봐"' } },
] as const;
