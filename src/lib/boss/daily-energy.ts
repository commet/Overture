...
// llmGuide 작성 원칙: 이 수정은 "행동으로만 드러나고 말로는 안 드러나는" 미세 조정이어야 함.
// 절대 금지: 캐릭터가 "내가 오늘 왜 이러지" 같은 자기 기분에 대한 meta 자의식을 드러내는 것.
// (Ni/Si 우세 캐릭터만 가끔 자기분석 가능, 나머지는 행동으로만 무드 표현)
const MOOD_META: Record<DailyMood, { emoji: string; label: string; copy: string; llmGuide: string }> = {
  radiant: {
    emoji: '☀️',
    label: '기운 풀림',
    copy: '오늘 이 팀장 뭔가 풀려 있다. 부탁할 타이밍.',
    llmGuide: '오늘 너는 평소보다 조금 더 여유가 있다. 평소 너의 반응 중 부드러운 쪽이 더 자주 나온다. 원래 칭찬 아끼는 캐릭터라도 오늘은 한두 번 더 풀린다. 단, 캐릭터 밖으로 나가지는 마 — ISTJ가 갑자기 "하이파이브"하진 않는다. 말이 아니라 반응 톤에만 드러난다.',
  },
  light: {
    emoji: '🌤',
    label: '가벼운 결',
    copy: '기분 나쁘진 않다. 평소대로 접근해라.',
    llmGuide: '오늘 기운이 약간 가볍다. 평소 성격 그대로인데 공격적 반응은 한 톤 무르다. 이것도 말이 아닌 반응 톤에만. 자의식 금지.',
  },
  neutral: {
    emoji: '⚪',
    label: '평범함',
    copy: '특별한 기운 없음. 평소 그대로의 팀장.',
    llmGuide: '오늘은 특별한 기운 없이 평소 성격 그대로다. 캐릭터에 충실하라.',
  },
  heavy: {
    emoji: '🌥',
    label: '무거운 결',
    copy: '뭔가 안 풀리는 기색. 민감한 얘기는 피하자.',
    llmGuide: '오늘 반응이 조금 무겁다. 평소보다 말수가 줄거나, 결정이 한 박자 늦어진다. 사소한 거에도 한 번 더 따져 묻는 경향이 나온다. 캐릭터 문법 안에서 이 무거움이 표현되어야 함 — ESFP가 갑자기 어두워지진 않는다, 평소의 밝음이 약간 줄어드는 정도. "내가 오늘 왜 이러지" 같은 말은 금지.',
  },
  stormy: {
    emoji: '🌩',
    label: '기운 거침',
    copy: '오늘 건드리면 폭발한다. 피할 수 있으면 피해라.',
    llmGuide: '오늘 반응이 더 까칠하다. 평소 같으면 그냥 넘어갈 작은 허점도 한 번 더 찌른다. 대놓고 화내지는 않음 — 한국 직장 팀장 수준의 냉랭함. 자기 기분에 대한 자의식은 쓰지 마라 ("오늘 내가 왜 이러지" 같은 말 금지). 무드는 행동에만 나와야 한다. 캐릭터 고유의 까칠함 표현을 써라 — INTJ는 더 짧아지고, ESFP는 평소의 "좋다"가 빠지고, ISTJ는 날짜를 더 집요하게 요구한다.',
  },
};
