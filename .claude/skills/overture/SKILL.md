...
- question: "뭘 해야 하는데 어디서 시작할지 모르겠어? 편하게 말해 — 횡설수설해도 내가 정리할게."
- header: "상황"
- options:
...
사용자가 선택하면 → 2가지를 한 번에 묻는다:
"구체적으로 어떤 거야? 그리고 평소에 뭘 하는 사람이야? (예: 백엔드 개발자, 디자이너, PM 등)"

**사용자 역할(user_role) 파악은 필수.** 어휘 브릿지, 질문 어투, 역번역의 핵심 입력.
입력에서 역할이 이미 드러나면 ("개발자인데...", "PM인데...") 별도로 묻지 않는다.
...
**고정 시퀀스가 아니다.** 이미 아는 건 건너뛴다.

| 우선순위 | 빠진 정보 | 왜 중요 | 질문 예시 |
|---------|----------|--------|----------|
| 1 | **구체적 내용** (도메인) | 이게 없으면 구체적으로 못 씀 | "어떤 영역이야?" |
| 1.5 | **사용자 역할** | 어휘 브릿지 수준 결정 | "평소에 뭘 하는 사람이야?" |
| 2 | **판단자** (누가 검토) | 문서의 톤과 깊이 결정 | "이걸 누가 판단해?" |
| 3 | **핵심 맥락** (배경) | 왜 이걸 해야 하는지 | "왜 이게 지금 나온 거야?" |
